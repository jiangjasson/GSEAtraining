...
output:
  html_document:
    ...
    toc_float: true
vignette: >
  %\VignetteIndexEntry{Topic 1-01: GO gene sets}
...
The **GO.db** package is built on top of a low-level infrastructure package **AnnotationDbi**. Thus, almost all
...
<img src="select_interface.png" width="600px" />
...
variables are actually redundant, e.g., `GOBPOFFSPRING` can be
constructed from `GOBPCHILDREN` recursively. However, these pre-computated objects will
...
has no child term (in this scenario, using `character(0)` instead of `NA` might be more proper).
...
For querying single GO terms, you don't need to explicitly convert the object to a list, 
but using `[[` instead.
...
1. For each term, we can recursively add child terms until reaching the leaf terms. This method traverses the GO DAG
from top (root) to bottom (leaves) in a depth-first search manner.
...
This method is very slow due to that a term may have multiple parents. Then a parent-child relation may be repeated visited for
terms close to root.

2. Or we traverse the DAG from bottom to top, and we recursively add the set of offsprings of a term to its parent terms. This method
also sues `GOBPPARENTS`.
...
which are usually synchronized between major public databases such as NCBI.
...
of providing mappings from Entrez gene IDs to a variaty of other databases. 
...
all its offspring terms. For example if term A is a parent of term B where A is more general,
genes with function B should also have function A. Thus `org.Hs.egGO2ALLEGS` is
the correct object for GO gene sets.
...
A gene can be duplicatedly annotated to a GO term with different evidence code. Thus,
...
Note the column `ONTOLOGYALL` can also be a valid "keytype" column, then actually
we can do "select all genes and GO terms where ontology is BP":
