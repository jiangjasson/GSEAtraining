...
The string `pathway` means to use the "pathway" database from KEGG. You can replace other databases on KEGG
to get other types of gene sets. Useful databases are:

- `network`, `module`, they are like sub-pathways.
- `ko`: KEGG orthology, a hierarchical list of vocabularies to describe biological functions on different levels.
- `disease`: disease-related genes.
- `drug`: drug-related genes.

But when applying the `list` API, no organism needs to be specified.

```
https://rest.kegg.jp/list/ko
https://rest.kegg.jp/list/network
https://rest.kegg.jp/list/drug
```
...
Then go to https://www.kegg.jp/pathway/`r pathway_id` and paste the color table there.
