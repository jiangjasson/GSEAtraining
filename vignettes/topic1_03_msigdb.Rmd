...
Since reading `.gmt` file is a basic task, we can wrap the code into a function:
...
We always recommend to use Entrez IDs as the internal gene IDs for analysis. In the previous practice for COVID-19 gene sets, genes are in symbols.
Can you convert them to gene sets where genes are in Entrez IDs?

You can directly use the `gs` object generated from the previous solution code.
...
gs2 = lapply(gs, function(x) {
    x2 = mapIds(org.Hs.eg.db, keys = x, keytype = "SYMBOL", column = "ENTREZID")
    unique(x2[!is.na(x2)])
})
gs2[1:2]
```

Method 2: use `map_to_entrez_id()` to generating mappings only to protein-coding genes.
