---
title: "Topic 1-07: Generate gene sets for other organisms by mapping to orthologues"
...
We have the mappings from human to panda, now we can construct the hallmark gene sets for panda. First let’s obtain the hallmark gene sets for human. The `get_msigdb()` function is from the **GSEAtutorial** package.

```{r}
library(GSEAtutorial)
gs_human = get_msigdb(version = "2024.1.Hs", collection = "h.all")
```
...
In **GSEAtutorial**, there is already a helper function `gs_map_to_orthologues()`. The gene IDs in the source gene sets must be Entrez IDs.

```{r}
gs_rat = gs_map_to_orthologues(gs_human, from = "Homo.sapiens", to = "Rattus.norvegicus")
gs_rat[1:2]
```
...
```{r}
gs_dolphin = gs_map_to_orthologues(gs_human, from = "Homo.sapiens", to = "Tursiops.truncatus")
```
