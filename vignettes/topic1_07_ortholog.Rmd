...
We have the mappings from human to panda, now we can construct the hallmark gene sets for panda. First let’s obtain the hallmark gene sets for human. The `get_msigdb()` function is from the **GSEAtopics** package.
...
In **GSEAtopics**, there is already a helper function `gs_map_to_orthologues()`. The gene IDs in the source gene sets must be Entrez IDs.
