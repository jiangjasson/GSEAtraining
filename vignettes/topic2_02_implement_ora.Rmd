...
universe = protein_coding_genes(org.Hs.eg.db)
```
...
ora(genes, gs_go, universe) |> head()
```

Or we can do it in a vectorized way. We also wrap the results as a data frame not only reporting p-values.
...
In many cases, the tests are applied for a huge number of times (> thousands). Also the methods for optimization can
be applied to a large number of other scenarios.
...
`ora()` is more than 40x faster than `ora_old()`! We have a very faster implementation of ORA 
...
And this is how `GSEAtopics::ora()` is optimized.
