...
we change the direction of the comparison to let "WT" as the treatment then the p53hypoxiaPathway gene set 
...
$$ P(X \ge x) \textnormal{ where } X = \max \left( F(g:g \in S) - F(g:g \notin S) \right) $$
...
$$ f_{1p} = \frac{1}{S} \sum_{i=1}^p I(g_i \in G_k)\cdot |t_i|^a \textnormal{ where } S = \sum_{i=1}^{N_k} I(g_i \in G_k)\cdot |t_i|^a $$

where $a$ is the power of the weight.
...
```{r}
calculate_es_v2_gene_perm_two_sided = function(s, geneset, perm = FALSE, plot = FALSE, power = 1, method = "std") {
...
a z-score `(es-mean(es_ran))/sd(es_rand)`.
