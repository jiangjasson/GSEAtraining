...
Heatmap(t(scale(t(expr[geneset, ]))), column_split = condition,
...
The ES score we have introduced in actually one-sided, i.e. we only look at the enrichment for up-regulated genes.
Basically, if there exist negative values in `f2 - f1`, it represents there is an enrichment for down-regulated genes.
The the enrichment score is the maximal between the positive ES and the negative ES (in their absolute values). The sign
of the maximal one is kept to represent the direction of the differential expression in the gene set.

```{r}
calculate_es_v2_gene_perm_two_sided = function(s, geneset, perm = FALSE, plot = FALSE, power = 1) {
...
The calculation of the p-value is a little bit different. As ES is actually from two different ES scores, the 1000 random ES
should be split into two distributions. If ES is positive, then we should only look at the null ES where they are positive.
And so is for the negative ES.

```{r}
hist(es_rand)
sum(es_rand > es)/sum(es_rand > 0)
es/mean(es_rand[es_rand > es])
```

In this case, z-score might be confusing as it measures how far it is from the positive null ES (to the right) or the negatice null ES (to the left).
