---
title: "Topic 6-02: Cluster and summarize"
...
output:
  html_document:
    ...
    toc_float: true
vignette: >
  %\VignetteIndexEntry{Topic 6-02: Cluster and summarize}
...
The block-pattern is very obvious. We can split it by hierarchical clustering + cutree, kmeans or any clustering methods.
...
We can see "CDKN1A" is in all 8 genesets, "MDM2" is in 7 out of 8 gene sets, and "BAX" is in 6 out of 8 gene sets. So
...
```{r}
ht = Heatmap(sm, name = "jaccard", col = c("white", "red"), 
...
and associate co-expressed pathways to a group of co-expressed genes.
...
$\mathrm{IC}(\mathrm{MICA})$ is replaced with $\mathrm{avg} \mathrm{IC}(\mathrm{CA})$, i.e. average non-zero IC of all common ancestors.
...
You can see there are around 900 GO terms under the cutoff `p_adjust < 0.001`. I suggest to set a cutoff
for `padj_cutoff` to let the number of GO terms be in `[100, 1000]`.
...
map = GSEAtopics::map_to_entrez_id("SYMBOL", org.Hs.eg.db)
